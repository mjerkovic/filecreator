...
TODAY=`date --date "$date" +"%d/%m/%Y"`

FILE_NAME=$1_$2_$3

csvFile=$DIR_NAME/$FILE_NAME.csv
echo "\"Document No\",\"Revision\",\"Title\",\"Type\",\"Status\",\"Discipline\",\"Attribute 1\",\"File\",\"Print Size\",\"Date Created\",\"Revision Date\",\"Supersede\"" > $csvFile

for i in `seq $2 $3`; 
do
  fn=$DIR_NAME/F$i.txt
  echo $i > $fn
  echo "\""$PREFIX"-"$(printf "%06d" $i)"\",\"A\",\"Test Doc "$i"\",\"Drawing\",\"Draft\",\"Structural\",\"Horizon Bar\",\"F"$i".txt\",\"A4\",\"$TODAY\",\"$TODAY\",," >> $csvFile
done
